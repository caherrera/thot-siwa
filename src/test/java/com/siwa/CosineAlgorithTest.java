...
public class CosineAlgorithTest {
    ...
    public void testCosine() throws Exception {
        String seed = "cosine";
        String comparator = "cosine";
        /* The second parameter indicates whether redirects must be followed or not */
        GetMethod get = client.get("/api/strings/" + seed + "/operations?operation=compare&with=" + comparator + "&criteria=cosine", false);
        ...
        response.put("criteria", "cosine");
        response.put("similarity", algorithms.get("cosine").calcule(seed, comparator));
        // Verify the response
    ...
    @Test
    public void testCosineAlphanumeric() throws Exception {
        ...
        URLEncoder.encode(comparator, "UTF-8") + "&criteria=cosine", false);
